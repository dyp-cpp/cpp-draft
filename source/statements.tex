...
\begin{codeblock}
{
  @\grammarterm{for-init-statement}@
  while( @\grammarterm{condition}@ ) {
    @\grammarterm{statement}@
...
let \term{range-init} be equivalent to the \grammarterm{expression} surrounded
...
let \term{range-init} be equivalent to the \grammarterm{braced-init-list}. In each case, a
...
\grammarterm{expression}, and \term{begin-expr} and \term{end-expr} are
determined as follows:

\begin{itemize}
\item if \tcode{_RangeT} is an array type, \term{begin-expr} and \term{end-expr} are
...
\tcode{begin} and \tcode{end} are looked up in the scope of class \tcode{_RangeT}
...
\item otherwise, \term{begin-expr} and \term{end-expr} are \tcode{begin(__range)}
