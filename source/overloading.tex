...
if
\tcode{F}
is a static member function, \ICS{1}{F} is defined such that
\ICS{1}{F} is neither better nor worse than \ICS{1}{G}
for any function
\tcode{G},
and, symmetrically, \ICS{1}{G} is neither better nor worse than
\ICS{1}{F}\footnote{If a function is a static member function, this
...
for all arguments
$i$,
\ICS{i}{F1} is not a worse conversion
sequence than \ICS{i}{F2}, and then
\begin{itemize}

\item
for some argument
$j$,
\ICS{j}{F1} is a better conversion
sequence than \ICS{j}{F2}, or, if not that,
...
other user-defined conversion sequence\cprotect\footnote{The ambiguous conversion
...
void f(C) { }
B b;
f(b);  // ambiguous because \tcode{b} $\to$ \tcode{C} via constructor and
       // \tcode{b} $\to$ \tcode{A} via constructor or conversion function.
\end{codeblock}
...
\emph{Lvalue Transformation},
\emph{Promotion}
or
\emph{Conversion},
\emph{Qualification Adjustment}.
\exitnote
...
No conversions required         &   Identity                                   &   \multirow{5}{*}{Exact Match}   &                       \\ \cline{1-2}\cline{4-4}
Lvalue-to-rvalue conversion     &   \multirow{3}{*}{Lvalue Transformation}     &                                  &   \ref{conv.lval}     \\ \cline{1-1}\cline{4-4}
Array-to-pointer conversion     &                                              &                                  &   \ref{conv.array}    \\ \cline{1-1}\cline{4-4}
...
Integral promotions             &   \multirow{2}{*}{Promotion}                 &   \multirow{2}{*}{Promotion}     &   \ref{conv.prom}     \\ \cline{1-1}\cline{4-4}
Floating point promotion        &                                              &                                  &   \ref{conv.fpprom}   \\ \hline
Integral conversions            &   \multirow{6}{*}{Conversion}                &   \multirow{6}{*}{Conversion}    &   \ref{conv.integral} \\ \cline{1-1}\cline{4-4}
...
Pointer conversions             &                                              &                                  &   \ref{conv.ptr}      \\ \cline{1-1}\cline{4-4}
...
\begin{codeblock}
@\cvqual{vq}\space\placeholder{T}@& operator++(@\cvqual{vq}\space\placeholder{T}@&);
@\placeholder{T}@ operator++(@\cvqual{vq}\space\placeholder{T}@&, int);
\end{codeblock}
...
\begin{codeblock}
@\cvqual{vq} \placeholder{T}@& operator--(@\cvqual{vq}\space\placeholder{T}@&);
@\placeholder{T}@ operator--(@\cvqual{vq}\space\placeholder{T}@&, int);
\end{codeblock}
...
\placeholder{T} that does not have cv-qualifiers or a \grammarterm{ref-qualifier},
there exist candidate operator functions of the form
...
\pnum
For every type \placeholder{T} there exist candidate operator functions of the form
...
\begin{codeblock}
@\placeholder{T}@ operator@\tilde@(@\placeholder{T}@);
\end{codeblock}
...
\begin{codeblock}
@\cvqual{cv12}\space\placeholder{T}@& operator->*(@\cvqual{cv1}\space\placeholder{C1}@*, @\cvqual{cv2}\space\placeholder{T}\space\placeholder{C2}@::*);
\end{codeblock}
...
\begin{codeblock}
@\placeholder{LR}@      operator*(@\textit{L}@, @\textit{R}@);
@\placeholder{LR}@      operator/(@\textit{L}@, @\textit{R}@);
@\placeholder{LR}@      operator+(@\textit{L}@, @\textit{R}@);
@\placeholder{LR}@      operator-(@\textit{L}@, @\textit{R}@);
...
\begin{codeblock}
@\cvqual{vq}\space\placeholder{L}@&   operator=(@\cvqual{vq}\space\placeholder{L}@&, @\placeholder{R}@);
@\cvqual{vq}\space\placeholder{L}@&   operator*=(@\cvqual{vq}\space\placeholder{L}@&, @\placeholder{R}@);
@\cvqual{vq}\space\placeholder{L}@&   operator/=(@\cvqual{vq}\space\placeholder{L}@&, @\placeholder{R}@);
@\cvqual{vq}\space\placeholder{L}@&   operator+=(@\cvqual{vq}\space\placeholder{L}@&, @\placeholder{R}@);
@\cvqual{vq}\space\placeholder{L}@&   operator-=(@\cvqual{vq}\space\placeholder{L}@&, @\placeholder{R}@);
...
\pnum
For every pair (\placeholder{T}, \cvqual{vq}), where \placeholder{T} is any type and \cvqual{vq} is either
